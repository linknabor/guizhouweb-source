        ...
        <mt-tab-container-item id="d">
          ...
          <div class="btn-fixed">
            <div
              class="fl select-btn"
              v-show="quan1"
              :class="{allSelected:quickAllselect}"
              @click="allSelect(quickBillInfo,'quickAllselect')"
            >全选&nbsp;</div>
...
export default {
  ...
  methods: {
        ...
        function() {
          if (vm.res.success) {
            ...
            if (vm.is_null == "0") {
              ...
              vm.startData = vm.res.result.start_date;
              vm.endData = vm.res.result.end_date;
              vm.startData = vm.startData.replace(
                /^(\d{4})(\d{2})(\d{2})$/,
                "$1-$2-$3"
              );
              vm.endData = vm.endData.replace(
        ...
        function() {
          if (vm.queryBillInfo.success) {
            if (vm.queryBillInfo.result == null) {
              ...
              vm.park_discount_rule_conf =
                vm.queryBillInfo.result.park_discount_rule_conf; //物业减免
              vm.park_discount_rule =
                vm.queryBillInfo.result.park_discount_rule;
    ...
    pays(list, allPrice, allselect) {
      ...
      //跳转支付
      let url =
        vm.config.wuye_payment.payment +
        "billIds=" +
        bills +
        "&stmtId=" +
        vm.stmtId +
